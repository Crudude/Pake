...
import {
  ...
  closeClientFile, reopenClient,
  deleteClient, changeClientType, addSessionType, updateSessionType,
  removeSessionType, setAssignmentSession,
} from '../actions.js';
...
function saveField(clientId, fieldKey, apply) {
  fieldSaver(`${clientId}:${fieldKey}`, (s) => {
...
export function renderDrawer(els, ctx) {
  ...
  drawer.innerHTML = `
    ...
    <section class="drawer-section">
      ...
      ${fixed ? `
        <div class="slot-list">
          ${slots.length ? slots.map((a) => slotRow(a, c, state)).join('')
  ...
  drawer.querySelector('[data-act="close"]').addEventListener('click', ctx.closeDrawer);
  ...
  if (!drawer.dataset.focusFlushWired) {
    ...
    drawer.addEventListener('focusout', () => setTimeout(() => fieldSaver.flush(), 0));
  }
  ...
  drawer.querySelector('[data-act="formulation"]')?.addEventListener('click', () => {
  ...
  if (placeBtn) placeBtn.addEventListener('click', () => ctx.startPlacement(c.id));

  drawer.querySelector('[data-act="add-session"]').addEventListener('click', () => {
    addSessionType(c.id);
  });
    ...
    for (const input of row.querySelectorAll('[data-sf]')) {
      ...
      if (input.tagName === 'SELECT') {
        input.addEventListener('change', () => {
          const value = field === 'duration' ? Number(input.value) : input.value;
  ...
  drawer.querySelector('[data-f="schedulingNotes"]').addEventListener('input', (e) => {
    const value = e.target.value;
    saveField(c.id, 'schedulingNotes', (cl) => { cl.schedulingNotes = value; });
  });
  ...
  for (const area of drawer.querySelectorAll('[data-cp]')) {
    area.addEventListener('input', (e) => {
      ...
      const value = e.target.value;
      saveField(c.id, `cp:${key}`, (cl) => { cl.casePlan[key] = value; });
    });
  }
  ...
  for (const row of drawer.querySelectorAll('[data-slot]')) {
    ...
    row.querySelector('[data-act="dur"]').addEventListener('click', () => cycleDuration(id));
    row.querySelector('[data-act="rm"]').addEventListener('click', () => removeAssignment(id));
    const sessSel = row.querySelector('[data-slot-sess]');
    if (sessSel) {
      sessSel.addEventListener('change', () => setAssignmentSession(id, sessSel.value));
    }
  }
