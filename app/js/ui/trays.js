...
export function renderRail(el, ctx) {
  ...
  if (!flexible.length) {
    flexZone.innerHTML =
      '<p class="tray-empty">No monthly or self-booking clients.</p>';
  }
  for (const c of flexible) {
    ...
    item.innerHTML = `
      ...
      <span class="meta">${TYPE_LABEL[c.type]}${
        c.schedulingNotes
  ...
  for (const c of paused) {
    ...
    const meta = [
      ...
      c.paused?.expectedReturn
        ? `back ${escapeHTML(c.paused.expectedReturn)}`
        : "",
    ]
      .filter(Boolean)
      .join(" &middot; ");
  ...
  for (const w of state.waitlist) {
    ...
    item
      .querySelector('[data-act="wl-remove"]')
    ...
    item
      .querySelector('[data-act="wl-promote"]')
      .addEventListener("click", () => {
  ...
  el.querySelector('[data-act="wl-add"]').addEventListener(
    "click",
    async () => {
  ...
  pausedGroup.addEventListener("dragover", (e) => {
    ...
    const target = clientId
      ? state.clients.find((x) => x.id === clientId)
      : null;
  ...
  pausedGroup.addEventListener("dragleave", () =>
