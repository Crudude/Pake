...
export function renderTraining(el, ctx) {
  ...
  const items = [...state.training].sort(
  ...
  for (const card of el.querySelectorAll("[data-training]")) {
    ...
    card
      .querySelector('[data-act="status"]')
      .addEventListener("change", (e) => {
        const status = e.target.value;
        mutate((s) => {
          s.training.find((x) => x.id === id).status = status;
        });
      });
    ...
    goal.addEventListener("change", () => {
      ...
      mutate(
        (s) => {
          s.training.find((x) => x.id === id).goal = value;
        },
        { skipPage: true },
      );
    });
    ...
    target.addEventListener("change", () => {
      ...
      mutate(
        (s) => {
          s.training.find((x) => x.id === id).target = value;
        },
        { skipPage: true },
      );
    });
    ...
    input.addEventListener("keydown", (e) => {
      ...
      mutate((s) => {
        s.training
          .find((x) => x.id === id)
          .progress.unshift({ date: todayISO(), text });
      });
    });
    ...
    card
      ...
      .addEventListener("click", async () => {
        ...
        mutate((s) => {
          s.training = s.training.filter((x) => x.id !== id);
        });
