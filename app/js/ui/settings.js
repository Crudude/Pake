...
export function renderSettings(el, ctx) {
  ...
  if (syncFile) {
    syncFile.addEventListener('click', closeAnd(async () => {
      let handle;
      try {
  ...
  el.querySelector('[data-act="jane-import"]').addEventListener('click', () => janeFile.click());
  ...
  el.querySelector('[data-act="recover"]').addEventListener('click', closeAnd(() => recoverPreDelete(ctx)));

  el.querySelector('[data-act="wipe"]').addEventListener('click', closeAnd(async () => {
    const ok = await openModal({
...
async function handleExistingFile(envelope, handle, ctx) {
  const when = envelope.savedAt ? new Date(envelope.savedAt).toLocaleString() : 'unknown time';
  ...
  if (overwrite) {
    ...
    toast('File linked, nothing written — the next save will ask about the conflict');
  }
}
...
async function recoverPreDelete(ctx) {
  ...
  if (validEnvelope(env)) {
    ...
    if (session.encrypted && session.role !== 'therapist') {
      toast('That copy holds clinical content — unlock with the therapist passphrase to recover it', 'warn');
      return;
    }
    ...
    if (!session.encrypted || !session.keys) {
      toast('That copy is encrypted — set up encrypted sync with the same passphrases first', 'warn');
      return;
    }
