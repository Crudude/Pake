...
const TYPE_LABEL = { weekly: 'Weekly', biweekly: 'Every other week', monthly: 'Monthly', self: 'Self-booking' };
...
export function renderRoster(el, ctx) {
  ...
  if (!state.clients.length) {
    ...
    el.querySelector('[data-act="add"]').addEventListener('click', ctx.addClientFlow);
    el.querySelector('[data-act="sample"]').addEventListener('click', ctx.loadSample);
    return;
  }
  ...
  const matches = (c) => c.name.toLowerCase().includes(q)
  ...
  const foundPaused = q ? sorted.filter((c) => c.status === 'paused').filter(matches) : [];
  ...
  const makeItem = (c, hintOverride = null) => {
    ...
    const hint = hintOverride
      ?? (c.status === 'paused' ? 'paused'
        : c.status === 'closed' ? 'closed'
          : (fixed ? summarizeSlots(state, c.id) : 'tray'));
    ...
    item.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); ctx.openClient(c.id); }
    });
    if (item.draggable) {
  ...
  if (closedList) {
    for (const c of closed) {
      closedList.append(makeItem(c, c.closed?.since ? `since ${c.closed.since.slice(5)}` : 'closed'));
    }
  }
