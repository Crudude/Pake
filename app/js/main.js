...
const PAGES = [
  {
    ...
    header: () => ({
      title: "Weekly <em>booking</em>",
      sub: "review the week · book in Jane · mark it here",
    }),
  },
  {
    id: "formulation",
    ...
    header: (s) => {
      ...
      return {
        title: "Case <em>conceptualization</em>",
        sub: n ? `${n} living document${n === 1 ? "" : "s"}` : "",
      };
    },
  },
  {
    ...
    header: (s) => {
      ...
      return {
        title: "Practice <em>tasks</em>",
        sub: open
          ? `${open} open · quietest first, ready for the meeting`
          : "all clear",
      };
    },
  },
  {
    ...
    header: (s) => {
      ...
      return {
        title: "Reading <em>list</em>",
        sub: `${now} in hand · ${queued} waiting`,
      };
    },
  },
  {
    ...
    header: (s) => {
      const active = s.training.filter(
        (t) => t.status === "in-progress",
      ).length;
      return {
        title: "Training <em>&amp; growth</em>",
        sub: active ? `${active} in progress` : "",
      };
    },
  },
];
...
const ctx = {
  ...
  get isAdmin() {
    return session.encrypted && session.role === "admin";
  },
  ui,
  rerender: () => renderAll(),
  openClient(id) {
    ui.drawerClientId = id;
...
function renderBoardTools(state) {
  ...
  els.boardTools.innerHTML = `
    <div class="seg" role="group" aria-label="Week view">
      ${views
        .map(
          ([v, label]) =>
            `<button type="button" data-view="${v}"${v === mode ? ' class="is-active"' : ""}>${escapeHTML(label)}</button>`,
        )
        .join("")}
    </div>
  ...
  els.boardTools
    .querySelector('[data-act="block-time"]')
  ...
  for (const btn of els.boardTools.querySelectorAll("[data-view]")) {
    btn.addEventListener("click", () => {
      ...
      mutate((s) => {
        s.settings.viewMode = btn.dataset.view;
      });
    });
  }
}
...
function renderHeader(state) {
  ...
  if (page) {
    ...
    els.weekHeading.innerHTML = `Week of ${parts.weekOf} &middot; <em>${escapeHTML(parts.parityHeading)}</em>`;
  ...
  els.backupStatus.textContent =
    age === null
      ...
      : age <= 0
        ? "Backed up today"
        : `Backed up ${age}d ago`;
  els.backupStatus.classList.toggle(
    "is-stale",
    age === null ? !!state.clients.length : age >= 7,
  );
}
...
function renderPlacementBar(state) {
  if (ui.placingBlock) {
    ...
    els.placementBar
      ...
      .addEventListener("click", ctx.endBlockPlacing);
    return;
  }
  ...
  els.placementBar
    .querySelector('[data-act="cancel"]')
    .addEventListener("click", ctx.endPlacement);
}
...
async function boot() {
  ...
  handlers.onSyncConflict = async (fileEnv) => {
    ...
    if (ok) {
      ...
      try {
        await saveSnapshot(fileEnv);
      } catch {
        /* conflict re-surfaces */
      }
      location.reload();
      return;
    }
    ...
    session.envelope.rev = Math.max(
      session.envelope.rev ?? 0,
      fileEnv.rev ?? 0,
    );
  ...
  document.addEventListener("drop", (e) => {
    ...
    restoreFromFile(file)
      .then(() => {
        if (!session.locked && document.querySelector(".load-error"))
          location.reload();
      })
