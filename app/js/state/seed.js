...
export function sampleData() {
  ...
  const c = (fullName, type, sessions, extra = {}) =>
    newClient({
      name: fullName,
    ...
    ],
    {
      ...
      casePlan: {
        ...
        log: [
          {
            date: "2026-07-14",
            text: "Big step: said no to weekend overtime.",
          },
  ...
  const priya = c(
    "Priya Sharma",
    "weekly",
    [sess("Individual", "in-person", "EMDR")],
    { schedulingNotes: "Saturdays only." },
  );

  const ben = c("Ben Carter", "biweekly", [
    sess("Individual", "virtual", "DBT"),
  ]);
  const chloe = c(
    "Chloe Nguyen",
    "biweekly",
    [sess("Individual", "in-person", "Grief work")],
    {
      casePlan: {
        workingOn: "Grief work — first year.",
        nextSession: "",
        longTermGoals: "",
        log: [],
      },
  ...
  const dana = c(
    "Dana Wolfe",
    "biweekly",
    [sess("Individual", "virtual", "CBT")],
    { schedulingNotes: "Shares the Tue 9 slot with BCa, alternate weeks." },
  );
  const omar = c("Omar Haddad", "biweekly", [
    sess("Trauma work", "in-person", "DBT-PE", 90),
  ]);
  const lena = c("Lena Brooks", "biweekly", [
    sess("Couples", "virtual", "Couples"),
  ]);
  const farah = c(
    "Farah Aziz",
    "biweekly",
    [sess("Check-in", "virtual", "DBT", 30)],
  ...
  const iris = c(
    ...
    [sess("Individual", "in-person", "CBT")],
    {
      ...
      paused: {
        since: "2026-06-02",
        expectedReturn: "September",
        note: "Away for the summer; keep her Thursday in mind.",
      },
    },
  );
