...
export function normalizeData(data) {
  ...
  if (
    !s.parityNames ||
    typeof s.parityNames !== "object" ||
    Array.isArray(s.parityNames)
  ) {
    s.parityNames = { even: "Even", odd: "Odd" };
  }
  s.parityNames.even =
    String(s.parityNames.even ?? "Even")
      .trim()
      .slice(0, 14) || "Even";
  s.parityNames.odd =
    String(s.parityNames.odd ?? "Odd")
      .trim()
      .slice(0, 14) || "Odd";
  if (s.lastBackupAt === undefined) s.lastBackupAt = null;
  if (
    s.viewMode === undefined ||
    !["split", "even", "odd"].includes(s.viewMode)
  )
    s.viewMode = "split";
  ...
  for (const r of data.reading) {
    if (!["to-read", "reading", "done"].includes(r.status))
      r.status = "to-read";
  ...
  for (const t of data.training) {
    ...
    if (!["planned", "in-progress", "done"].includes(t.status))
      t.status = "planned";
  ...
  for (const c of data.clients) {
    ...
    if (!["weekly", "biweekly", "monthly", "self"].includes(c.type))
      c.type = "biweekly";
...
function isEncryptedEnvelope(env) {
  return (
    !!env && env.format === "cadence-encrypted" && !!env.practice && !!env.roles
  );
}
...
export async function initStore() {
  ...
  if (session.handle) {
    ...
    if (session.filePermission)
      fileEnv = await readFileEnvelope(session.handle);
  }

  const res = await loadLatest(
    (e) => validEnvelope(e) || isEncryptedEnvelope(e),
  );
  ...
  if (isEncryptedEnvelope(chosen)) {
    ...
    session.fileRev = divergentTie
      ? -1
      : isEncryptedEnvelope(fileEnv)
        ? fileEnv.rev
        : null;
    session.fileDevice = divergentTie
      ? null
      : isEncryptedEnvelope(fileEnv)
        ? (fileEnv.deviceId ?? null)
        : null;
...
export async function unlockWithPassphrase(passphrase) {
  ...
  try {
    await kvSet("keyCache", await exportRawKeys(opened.keys));
  } catch {
...
export async function lockDevice() {
  ...
  if (session.encrypted && session.keys && !session.locked) {
    ...
    try {
      ...
      /* lock anyway; the last committed snapshot still stands */
    }
  }
  await kvDel("keyCache");
  location.reload();
...
export async function linkSaveFile(handle) {
  ...
  session.fileRev = null;
  session.fileDevice = null;
  if (session.encrypted && session.envelope)
    await flushFileWrite(session.envelope);
...
export async function adoptWithPassphrase(envelope, passphrase) {
  ...
  try {
    ...
    /* initStore retro-cleans on next boot */
  }
  return true;
}
