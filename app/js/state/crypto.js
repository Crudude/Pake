...
export function splitData(data) {
  ...
  for (const c of practice.clients) {
    clinical[c.id] = {
      casePlan: c.casePlan,
      formulation: c.formulation ?? null,
    };
  ...
  for (const role of ["therapist", "admin"]) {
    ...
    try {
      ...
      if (role === "therapist") {
        ...
        return {
          role,
          keys: { practiceKey, clinicalKey },
          data: mergeData(practice, clinical),
        };
      }
...
export async function exportRawKeys(keys) {
  const out = {
    practice: await crypto.subtle.exportKey("raw", keys.practiceKey),
  };
  if (keys.clinicalKey)
    out.clinical = await crypto.subtle.exportKey("raw", keys.clinicalKey);
  return out;
}
...
export async function decryptWithRawKeys(envelope, raw) {
  const importOne = (buf) =>
    crypto.subtle.importKey("raw", buf, { name: "AES-GCM" }, true, [
      "encrypt",
      "decrypt",
    ]);
  try {
...
export async function selfTest() {
    ...
    clients: [
      {
        ...
        casePlan: {
          ...
          nextSession: "",
          longTermGoals: "",
          log: [{ date: "2026-07-19", text: "SECRET NOTE" }],
        },
        formulation: { core: "SECRET FORMULATION" },
        schedulingNotes: "mornings",
      },
    ],
    assignments: [
      {
  ...
  const env = await createEncryptedEnvelope(
    data,
    "therapist-pass",
    "admin-pass",
  );
  ...
  if (
    t.role !== "therapist" ||
    t.data.clients[0].casePlan.workingOn !== "SECRET"
  )
    throw new Error("therapist unlock failed");

  const a = await unlockEnvelope(env, "admin-pass");
  if (a.role !== "admin" || a.data.clients[0].casePlan.workingOn !== "")
    throw new Error("admin must not see case info");
  if (a.data.clients[0].formulation !== null)
    throw new Error("admin must not see formulations");
  if (a.data.clients[0].schedulingNotes !== "mornings")
    throw new Error("admin must see scheduling notes");

  if ((await unlockEnvelope(env, "wrong")) !== null)
  ...
  if (t2.data.clients[0].casePlan.log[0].text !== "SECRET NOTE")
    throw new Error("clinical lost after admin save");
  if (t2.data.clients[0].formulation?.core !== "SECRET FORMULATION")
    throw new Error("formulation lost after admin save");
